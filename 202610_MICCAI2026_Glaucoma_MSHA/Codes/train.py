...
parser.add_argument('--train_csv', '-t_csv', metavar='NAME', default='',
                    help='dataset type (default: Enter train csv file path)')
parser.add_argument('--val_csv', '-v_csv', metavar='NAME', default='',
                    help='dataset type (default: Enter val csv file path)')
...
def main():
    ...
    dataset_train = create_dataset(
        args.dataset,
        root=args.data_dir, split=args.train_split, is_training=True,class_map=args.class_map,seed=args.seed,
        batch_size=args.batch_size, repeats=args.epoch_repeats, csv_path=args.train_csv,download=args.dataset_download,)
    
    dataset_eval = create_dataset(
        args.dataset, root=args.data_dir, split=args.val_split, is_training=False,class_map=args.class_map,
        batch_size=args.batch_size, csv_path=args.val_csv,download=args.dataset_download,)

    # added to spliy datatset during training
    train_split_new = 0.8
    ...
    datatset_val =  SubsetRandomSampler(val_indices)
    
    dataset_train, dataset_val = random_split(dataset_train, (1000, 118))
    '''
